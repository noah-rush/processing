...
class Spot{
  ...
  case 9: filler = color(244, 250, 88);
     ...
     // noStroke();
      ...
      stroke(filler, 180);
      strokeWeight(5);
      arc(x,y, size+10*(i+1), size +10*(i+1), 0, 2*PI);
    }
...
void move( boolean onoff, float lBounds, float rBounds){
    if( rightwall || leftwall){
    this.checkBoundaryCollision(lBounds, rBounds);
   ...
   void checkBoundaryCollision(float lBounds, float rBounds) {
     float r = size/2;
    if (position.x > rBounds-r) {
      position.x = rBounds-r;
      velocity.x *= -1;
    } 
    else if (position.x < r + lBounds) {
      position.x = r + lBounds;
      velocity.x *= -1;
    } 
...
boolean inline(){
  boolean a;
  if( abs((y - map(line*height/7 , 0, height, 100, height-100 )))<20){
  ...
  void moveToLines(int stripes, int orientation, float stroke, float speed, int totalpoints){
    ...
    if(orientation == 0){
      if( y - map(height/stripes * line, 0, height, 100, height-100 )<15){
        this.move(false, leftWall, rightWall);
        if(firsttime){
        this.setAngle(PI/2);
        firsttime = false;
        }
